---
title: 鱼与熊掌兼得:Shadow DOM + IIFE构建轻量级前端隔离沙箱
description: 使用Shadow DOM和IIFE代理技术构建轻量级、高性能的前端应用隔离沙箱,解决样式污染和DOM冲突问题
pubDate: 2025-08-25
tag:
  - javascript
  - vue
...
这是最初级的垃圾回收算法。此算法把确定对象是否仍然需要这个问题简化为确定对象是否仍有其他引用它的对象。如果没有指向该对象的引用，那么该对象称作“垃圾”或者可回收的。 比较简单的示例可以看[rust的智能指针](https://kaisery.github.io/trpl-zh-cn/ch15-06-reference-cycles.html)。
