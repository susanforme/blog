...
Provider是 Nest 的一个核心概念。许多基础的 Nest 类，例如service、(repository、factory和helper，都可以被视为provider。其核心思想在于，它可以作为依赖进行**注入**（inject），从而允许对象之间建立各种各样的关系。而将这些对象“组装”（wiringup）起来的职责，则主要由 Nest 运行时系统来处理。
...
| 属性 (`Property`) | 说明                                                                                                                               |
| :---------------- | :--------------------------------------------------------------------------------------------------------------------------------- |
| `providers`       | 将由 Nest 注入器实例化的provider集合，这些provider将至少在当前模块内共享。                                                         |
| `controllers`     | 在当前模块中定义的、必须被实例化的控制器集合。                                                                                     |
| `imports`         | 导入的模块列表。这些模块导出了在当前模块中所需的provider。                                                                         |
| `exports`         | 由当前模块提供、并应在其他导入了本模块的模块中可用的provider子集。你可以使用provider本身，也可以只使用它的令牌（`provide` 的值）。 |
