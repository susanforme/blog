...
podman和docker的主要区别在于，podman 不需要一个持续运行的守护进程来管理容器。相反，podman 直接与容器运行时（如 runc 或 crun）交互，这使得它更轻量级且更易于集成到现有的系统中。此外，podman 支持无根模式，这意味着用户可以在不需要超级用户权限的情况下运行容器，从而提高了安全性。Podman 还提供了与**Docker CLI 兼容的命令行界面** ，这使得从 Docker 迁移到 Podman 变得相对简单。许多常用的 Docker 命令在 Podman 中都有对应的命令，例如
